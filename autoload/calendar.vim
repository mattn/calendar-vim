...
"*****************************************************************
"* init_table_layout : compute layout for table (dir==2) display
"*****************************************************************
function! s:init_table_layout() abort
  ...
  let l:vrt = max([(l:height - &cmdheight - 3 - l:navifix) / 6 - 2, 0])
  let l:whitevrta = (l:whitehrz == '|') ? l:whitehrz : l:whitehrz[1:]
  ...
  return {
        \ 'width': l:width,
        \ 'whitehrz': l:whitehrz,
        \ 'whitevrta': l:whitevrta,
        \ 'whiteleft': l:whiteleft,
        \ 'whitevrt': l:whitevrta . l:whitevrt . l:whitevrt2 . "\n",
        \ 'whitevrtweeknm': l:whitevrt . l:whitevrt2 . "\n",
        \ 'borderhrz': l:borderhrz,
        \ 'fridaycol': (strlen(l:whitehrz) + 3) * 5 + strlen(l:whiteleft) + 1,
        \ 'saturdaycol': (strlen(l:whitehrz) + 3) * 6 + strlen(l:whiteleft) + 1,
        \ }
endfunction

"*****************************************************************
"* build_month_string : generate calendar string for one month
"*****************************************************************
function! s:build_month_string(vyear, vmnth, vtoday, dir, tbl) abort
  let l:vcolumn = 22
  let l:vmdays = s:month_days[a:vmnth - 1]
  let l:vparam = s:month_params[a:vmnth - 1]
  let l:vsmnth = s:month_names[a:vmnth - 1]
  let l:vleap = (a:vyear % 4 == 0 && a:vyear % 100 != 0) || a:vyear % 400 == 0
  if l:vleap
    if a:vmnth == 2
      let l:vmdays = 29
    elseif a:vmnth >= 3
  ...
  let l:vnweek = (a:vyear * 365) + l:vparam
        \ + (a:vyear/4) - (a:vyear/100) + (a:vyear/400)
  if l:vleap
  ...
  if exists('g:calendar_monday')
    let l:vnweek = (l:vnweek == 0) ? 6 : l:vnweek - 1
  endif
  ...
  if exists('g:calendar_weeknm')
    let l:vnweek_iso = exists('g:calendar_monday') ? l:vnweek : (l:vnweek + 6) % 7
    ...
    if l:viweek == 0
      let l:viweek = 52
      let l:vleap_prev = ((a:vyear-1) % 4 == 0 &&
            \ ((a:vyear-1) % 100 != 0 || (a:vyear-1) % 400 == 0))
      if l:vfweek == 2 || (l:vfweek == 1 && l:vleap_prev)
        let l:viweek = 53
      endif
    endif
    let l:vcolumn += (g:calendar_weeknm == 5) ? 3 : 5
  endif

  " header
  let l:vdisplay2 = a:vyear.'/'.a:vmnth.'('
  if exists('g:calendar_erafmt') && g:calendar_erafmt !~ "^\s*$"
        \ && g:calendar_erafmt =~ '.*,[+-]*\d\+'
    let l:veranum = substitute(g:calendar_erafmt,'.*,\([+-]*\d\+\)','\1','')
    if a:vyear+l:veranum > 0
      let l:vdisplay2 = substitute(g:calendar_erafmt,'\(.*\),.*','\1','')
            \ .(a:vyear+l:veranum).'/'.a:vmnth.'('
    endif
  endif
  ...
  let l:mname = (exists('g:calendar_mruler') && g:calendar_mruler !~ "^\s*$")
        \ ? get(split(g:calendar_mruler, ','), a:vmnth-1, '') : l:vsmnth
  let l:vdisplay2 .= l:mname . ')' . "\n"
  ...
  if exists('g:calendar_monday')
    let l:vwruler = strpart(l:vwruler, stridx(l:vwruler, ' ') + 1)
          \ . ' ' . strpart(l:vwruler, 0, stridx(l:vwruler, ' '))
  endif
  if a:dir == 2
    let l:whiteruler = substitute(substitute(a:tbl.whitehrz, ' ', '_', 'g'), '__', '  ', '')
    let l:vwruler = '| '.substitute(l:vwruler, ' ', l:whiteruler.' ', 'g').l:whiteruler
    let l:vdisplay2 .= a:tbl.whiteleft . l:vwruler . "\n"
  else
    let l:vdisplay2 .= ' ' . l:vwruler . "\n"
  endif
  if g:calendar_mark == 'right' && a:dir != 2
    let l:vdisplay2 .= ' '
  endif
  ...
  while l:vinpcur < l:vnweek
    if a:dir == 2
      let l:vdisplay2 .= (l:vinpcur % 7) ? a:tbl.whitehrz : (a:tbl.whiteleft . '|')
    endif
    let l:vdisplay2 .= '   '
    let l:vinpcur += 1
  endwhile
  ...
  while l:vdaycur <= l:vmdays
    ...
    let l:vtarget = printf('%d%02d%02d', a:vyear, a:vmnth, l:vdaycur)

    " sign
    if exists("g:calendar_sign") && g:calendar_sign != ""
      exe "let l:vsign = " . g:calendar_sign . "(l:vdaycur, a:vmnth, a:vyear)"
      if l:vsign != ""
    ...
    " day number with mark
    if g:calendar_mark == 'right'
      let l:vdisplay2 .= (l:vdaycur < 10 ? ' ' : '') . l:vdaycur
    elseif g:calendar_mark == 'left-fit' && l:vdaycur < 10
      let l:vdisplay2 .= ' '
    endif
    let l:vdisplay2 .= (l:vtarget == a:vtoday) ? '*'
          \ : (l:vsign != '') ? l:vsign : ' '
    if g:calendar_mark == 'left'
      let l:vdisplay2 .= (l:vdaycur < 10 ? ' ' : '') . l:vdaycur
    elseif g:calendar_mark == 'left-fit'
      let l:vdisplay2 .= l:vdaycur
    endif
    let l:vdaycur += 1

    " fix Gregorian
    if a:vyear == 1752 && a:vmnth == 9 && l:vdaycur == 3
      let l:vdaycur = 14
    endif
    ...
    if l:vinpcur % 7 == 0
      if exists('g:calendar_weeknm')
        if a:dir == 2
          let l:vdisplay2 .= a:tbl.whitehrz
        endif
        if g:calendar_mark != 'right'
          let l:vdisplay2 .= ' '
        endif
        let l:vdisplay2 .= s:format_weeknm(g:calendar_weeknm, l:viweek)
        let l:viweek += 1
  ...
  return {'display': l:vdisplay2, 'column': l:vcolumn, 'wruler': l:vwruler}
endfunction

"*****************************************************************
"* merge_displays : merge month displays according to direction
"*****************************************************************
function! s:merge_displays(dir, vdisplay1, vdisplay2, vcolumn, vmcnt, vheight, tbl) abort
  let l:lines1 = split(a:vdisplay1, "\n")
  let l:lines2 = split(a:vdisplay2, "\n")
  let l:vstrline = ''
  let l:vheight = a:vheight

  if a:dir == 1
    " horizontal
    let l:vtokline = 0
    while 1
      let l:vtoken1 = get(l:lines1, l:vtokline, '')
      let l:vtoken2 = get(l:lines2, l:vtokline, '')
      if l:vtoken1 == '' && l:vtoken2 == ''
        break
      endif
      while strlen(l:vtoken1) < (a:vcolumn+1)*a:vmcnt
        let l:vtoken1 .= (strlen(l:vtoken1) % (a:vcolumn+1) == 0) ? '|' : ' '
      endwhile
      let l:vstrline .= l:vtoken1 . '|' . l:vtoken2 . " \n"
      let l:vtokline += 1
    endwhile
    let l:vheight = l:vtokline
  elseif a:dir == 0 || a:dir == 3
    " vertical
    for l:line in l:lines1
      let l:vstrline .= l:line . "\n"
      let l:vheight += 1
    endfor
    if l:vstrline != ''
      let l:vstrline .= " \n"
    ...
    for l:line in l:lines2
      let l:padded = l:line
      while strlen(l:padded) < a:vcolumn
        let l:padded .= ' '
      endwhile
      let l:vstrline .= l:padded . "\n"
    endfor
    let l:vheight = len(l:lines2) + 1
  else
    " table (dir==2)
    let l:vtokline = 0
    while 1
      ...
      if l:vtokline >= 2
        if exists('g:calendar_weeknm')
          let l:vright = a:tbl.whitevrtweeknm
        elseif a:tbl.whitehrz == '|'
          let l:vright = a:tbl.whitevrt
        else
          let l:vright = ' ' . a:tbl.whitevrt
        endif
      else
      ...
      let l:vstrline .= l:vtoken1 . l:vtoken2 . l:vright
      let l:vtokline += 1
    endwhile
    let l:vheight = l:vtokline
  endif

  return [l:vstrline, l:vheight]
endfunction
...
function! calendar#action(...) abort
  " handle calendar list selection
  let l:text = getline(".")
  if l:text =~ "^( )"
    let l:list_idx = 0
    let l:curl = line(".") - 1
  ...
  call s:handle_day_action()
endfunction

"*****************************************************************
"* handle_navi : process navigation clicks
"*   returns 1 if navigation was handled, 0 otherwise
"*****************************************************************
function! s:handle_navi(navi) abort
  let l:navi_parts = split(g:calendar_navi_label, ',')
  let l:curl = line(".")
  let l:curp = getpos(".")
  let l:navi = a:navi

  if l:navi == '<' . get(l:navi_parts, 0, '')
...
"*****************************************************************
"* handle_day_action : resolve clicked day and invoke action
"*****************************************************************
function! s:handle_day_action() abort
  ...
  elseif b:CalendarDir == 1
    let l:dir = 'H'
    let l:cnr = col('.') - (col('.') % (exists('g:calendar_weeknm') ? 29 : 24)) + 1
...
function! calendar#show(...) abort
  ...
  if l:dir == 2
    let l:vmcntmax = 1
    let l:tbl = s:init_table_layout()
    let l:whitehrz = l:tbl.whitehrz
    let l:whitevrta = l:tbl.whitevrta
    let l:whiteleft = l:tbl.whiteleft
    let l:whitevrt = l:tbl.whitevrt
    let l:whitevrtweeknm = l:tbl.whitevrtweeknm
    let l:borderhrz = l:tbl.borderhrz
    let l:fridaycol = l:tbl.fridaycol
    let l:saturdaycol = l:tbl.saturdaycol
    let l:width = l:tbl.width
  else
  ...
  while l:vmcnt < l:vmcntmax
    ...
    let l:tbl_vars = (l:dir == 2)
          \ ? {'whitehrz': l:whitehrz, 'whiteleft': l:whiteleft,
          \    'whitevrtweeknm': l:whitevrtweeknm, 'whitevrt': l:whitevrt}
          \ : {}
    let l:month = s:build_month_string(l:vyear, l:vmnth, l:vtoday, l:dir, l:tbl_vars)
    let l:vdisplay2 = l:month.display
    let l:vcolumn = l:month.column
    let l:vwruler = l:month.wruler
    let [l:vdisplay1, l:vheight] = s:merge_displays(
          \ l:dir, l:vdisplay1, l:vdisplay2, l:vcolumn, l:vmcnt, l:vheight, l:tbl_vars)
    let l:vmnth = l:vmnth + 1
  ...
  call s:open_calendar_window(l:dir, l:vheight, l:vcolumn, l:vyear, l:vmnth)
  let b:CalendarDir = l:dir
  let b:CalendarYear = l:vyear_org
  let b:CalendarMonth = l:vmnth_org

  call s:render_navi(l:dir, l:vcolumn, get(l:, 'width', 0), l:vdisplay1)
...
"*****************************************************************
function! s:open_calendar_window(dir, vheight, vcolumn, vyear, vmnth) abort
  ...
  if l:vwinnum >= 0
    ...
    let l:vheight = a:vheight
    if exists('g:calendar_navi') && a:dir
      let l:vheight += (g:calendar_navi == 'both') ? 4 : 2
    endif
    ...
    elseif a:dir == 0
      silent execute 'to '.a:vcolumn.'vsplit __Calendar'
      setlocal winfixwidth
    elseif a:dir == 3
      silent execute 'bo '.a:vcolumn.'vsplit __Calendar'
      setlocal winfixwidth
    ...
    call s:CalendarBuildKeymap(a:dir, a:vyear, a:vmnth)
    setlocal noswapfile
    ...
    if a:dir != 2
      exe a:vcolumn + l:nontext_columns . "wincmd |"
    endif
  endif
...
"*****************************************************************
"* render_navi : render navigation labels and calendar content
"*****************************************************************
function! s:render_navi(dir, vcolumn, width, vdisplay1) abort
  if exists('g:calendar_navi')
    let l:navi_parts = split(g:calendar_navi_label, ',')
    let l:navi_label = '<' . get(l:navi_parts, 0, '') . ' '
          \ . get(l:navi_parts, 1, '') . ' '
          \ . get(l:navi_parts, 2, '') . '>'
    if a:dir == 1
      let l:navcol = a:vcolumn + (a:vcolumn - strlen(l:navi_label) + 2) / 2
    elseif a:dir == 0 || a:dir == 3
      let l:navcol = (a:vcolumn - strlen(l:navi_label) + 2) / 2
    else
      let l:navcol = (a:width - strlen(l:navi_label)) / 2
    endif
    let l:navcol = max([l:navcol, 3])

    if g:calendar_navi == 'top'
      execute "normal gg".l:navcol."i "
      silent exec "normal! a".l:navi_label."\<cr>\<cr>"
      silent put! =a:vdisplay1
    elseif g:calendar_navi == 'bottom'
      silent put! =a:vdisplay1
      silent exec "normal! Gi\<cr>"
      execute "normal ".l:navcol."i "
      silent exec "normal! a".l:navi_label
    elseif g:calendar_navi == 'both'
      execute "normal gg".l:navcol."i "
      silent exec "normal! a".l:navi_label."\<cr>\<cr>"
      silent put! =a:vdisplay1
      silent exec "normal! Gi\<cr>"
